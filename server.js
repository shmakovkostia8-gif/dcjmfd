...
function getRoom(roomId) {
    if (!rooms.has(roomId)) {
        rooms.set(roomId, {
            ...
            users: new Map(),
            // clientId (стабильный id браузерной сессии) -> текущий socket.id
            // нужен, чтобы при переподключении не плодить "дубликат себя" в комнате
            clientToSocket: new Map(),
            created: Date.now()
        });
    }
...
io.on('connection', (socket) => {
    ...
    socket.on('join', ({ roomId, name, color, clientId }) => {
        if (!roomId || typeof roomId !== 'string') return;

        currentRoom = roomId.toUpperCase().trim();
        myName = (name || 'Аноним').slice(0, 32);
        myColor = color && /^#[0-9A-Fa-f]{6}$/.test(color) ? color : '#d4820a';
        myClientId = (typeof clientId === 'string' && clientId) ? clientId.slice(0, 64) : null;

        const room = getRoom(currentRoom);

        // ── Фикс "дублирования себя" ──
        // Если этот же браузер/сессия (тот же clientId) уже подключён под другим
        // socket.id (например, после реконнекта), убираем старое подключение
        // СРАЗУ, не дожидаясь pingTimeout, и уведомляем остальных, что старый "слот" вышел.
        if (myClientId && room.clientToSocket.has(myClientId)) {
            const oldSid = room.clientToSocket.get(myClientId);
            if (oldSid && oldSid !== socket.id && room.users.has(oldSid)) {
                room.users.delete(oldSid);
                io.to(currentRoom).emit('peer:leave', { sid: oldSid, name: myName });
                const oldSocket = io.sockets.sockets.get(oldSid);
                if (oldSocket) oldSocket.disconnect(true);
                console.log(`♻️  Заменено старое подключение клиента ${myClientId}: ${oldSid} → ${socket.id}`);
            }
        }
        ...
        if (myClientId) room.clientToSocket.set(myClientId, socket.id);
    ...
    // ── WebRTC сигналинг ──
    // offer используется как для первичного соединения, так и для пересогласования
    // (например, когда собеседник включает демонстрацию экрана уже во время звонка)
    ...
    socket.on('disconnect', () => {
        if (currentRoom) {
            const room = rooms.get(currentRoom);
            if (room) {
                const hadUser = room.users.has(socket.id);
                room.users.delete(socket.id);

                // Чистим clientToSocket только если он всё ещё указывает на ЭТОТ сокет
                // (если клиент уже успел переподключиться, там уже новый sid — трогать не нужно)
                if (myClientId && room.clientToSocket.get(myClientId) === socket.id) {
                    room.clientToSocket.delete(myClientId);
                }
